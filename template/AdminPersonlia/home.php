<?php
   $jmlAdmin = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT count(a.jenis_admin) as 'jmlAdmin' FROM admin a inner join jenis_user j ON a.jenis_admin = j.id WHERE j.jenis = 'AdminPersonalia' group by a.jenis_admin"));
   $jmlAnggota = mysqli_fetch_assoc(mysqli_query($koneksi, "SELECT count(a.jenis_anggota) as 'jmlAnggota' FROM anggota a inner join jenis_user j ON a.jenis_anggota = j.id WHERE j.jenis = 'AdminPersonalia' AND a.status = 1 group by a.jenis_anggota"));
   ...
   $link = "AdminPersonalia.php?page=AdminPersonlia&page2=home";
